({
	doInit : function(component, event, helper) {
		if(component.get("v.records").length * component.get("v.fieldNames").length == 0){
			helper.getColumnHeader(component,helper);
	...
	handleRecodsChange : function(component, event, helper) {
		if( !event.getParam("oldValue") || (event.getParam("oldValue").length == 0 && event.getParam("value").length != 0) ){
			helper.populateMockRecord(component,helper);
		}
	},
	...
	handleComponentAction : function(component, event, helper){
		if(component 
				...
				if(actionName=='edit'){
					...
					helper.populateColumnChanges(component,helper);
					helper.saveData(component, helper);
				}
